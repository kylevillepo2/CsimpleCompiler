...
Symbol* SymScope::insert( char* name, Symbol * s )
{
    ...
    if(iret.second == true) {
        ...
        int aligned_size = (s->get_size() + 3) & ~3;  // Round up to 4-byte boundary
        m_scopesize += aligned_size;
        s->m_offset = m_scopesize;  // Negative offset from %ebp for local variables
